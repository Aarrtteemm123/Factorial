...
public class CalculationFactorial extends Thread {
    ...
    public CalculationFactorial(int n, int indexThread, int countThreads, Phaser phaser,ArrayList arrayList,String fileName) {
    ...
    private void saveResult(String fileName,double arr[])
    {
        try(FileWriter writer = new FileWriter(fileName,false)) {
            for (int i = 0; i < arr.length; i++)
            {
                if (i%110==0&&i!=0)
                    writer.write("\n");
                writer.write(String.valueOf((int) arr[i]));
            }
            writer.write("\n\nSize(num) : " + arr.length);
            writer.write("\nLenght(m) : " + (double) arr.length * 2.5 / 1000);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
