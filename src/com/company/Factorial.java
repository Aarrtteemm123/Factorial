    ...
    public void save(String fileName)
    {
        try(FileWriter writer = new FileWriter(fileName,false)) {
            for (int i = 0; i < factorial.length; i++)
            {
                if (i%110==0&&i!=0)
                    writer.write("\n");
                writer.write(String.valueOf((int) factorial[i]));
            }
            writer.write("\n\nSize(num) : " + factorial.length);
            writer.write("\nLenght(m) : " + (double) factorial.length * 2.5 / 1000);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public void save(String fileName,double time)
    {
        try(FileWriter writer = new FileWriter(fileName,false)) {
            for (int i = 0; i < factorial.length; i++)
            {
